...
def graphics_mode_switcher(graphics_mode, user_display_manager, enable_force_comp, coolbits_value, rtd3_value, use_nvidia_current):
    ...
    if graphics_mode == 'integrated':
        ...
        if logging.getLogger().level == logging.DEBUG:
            service = subprocess.run(["s6-rc", "-d", "change", "nvidia-persistenced"])
        else:
            service = subprocess.run(["s6-rc", "-d", "change", "nvidia-persistenced"],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        if logging.getLogger().level == logging.DEBUG:
            service = subprocess.run(["s6-rc", "-u", "change", "nvidia-persistenced"])
        else:
            service = subprocess.run(["s6-rc", "-u", "change", "nvidia-persistenced"],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if service.returncode == 0:
