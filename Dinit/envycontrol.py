...
SUPPORTED_DISPLAY_MANAGERS = ['gdm', 'gdm3', 'sddm', 'lightdm', 'manual']
RTD3_MODES = [0, 1, 2, 3]
...
def graphics_mode_switcher(graphics_mode, user_display_manager, enable_force_comp, coolbits_value, rtd3_value, use_nvidia_current):
    ...
    if graphics_mode == 'integrated':
        ...
        if logging.getLogger().level == logging.DEBUG:
            service = subprocess.run(["dinitctl", "disable", "nvidia-persistenced"])
        else:
            service = subprocess.run(["dinitctl", "disable", "nvidia-persistenced"],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        if logging.getLogger().level == logging.DEBUG:
            service = subprocess.run(["dinitctl", "enable", "nvidia-persistenced"])
        else:
            service = subprocess.run(["dinitctl", "enable", "nvidia-persistenced"],stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if service.returncode == 0:
        ...
        if user_display_manager == 'manual':
            print ("\033[31;1;3mIMPORTANT! - Please make sure you've added the appropriate xrandr script to your display manager - https://github.com/bayasdev/envycontrol/wiki/Frequently-Asked-Questions#what-to-do-if-my-display-manager-is-not-supported\033[0m")
...
def get_display_manager():
    try:
        ...
        logging.error("Display Manager was not specified")
        exit(1)
